...
from config_ga import (
    N_CAPAS_MAX, S_VERT_MIN_CM, R_LIBRE_CM, D_ESTRIBO_CM, ZONE_IDS,
)
...
if __name__ == '__main__':
    ...
    L = chrom_length(n_s)
    print(f"b={b}, n_slots={n_s}, chrom_length={L}, block={block_size(n_s)}")

    # Cromosoma aleatorio
    z_rand = np.zeros(L, dtype=np.int8)
    z_rand[0] = 1  # diam_A = 5/8"
    z_rand[1] = 2  # diam_B = 3/4"
    # Activar algunas barras del corrido capa_0
    z_rand[2] = 0   # slot 0: diam_A, ON
    z_rand[3] = 1
    z_rand[4] = 0   # slot 1: diam_A, ON
    z_rand[5] = 1
    z_rand[2 + (n_s - 1) * 2] = 0   # esquina derecha
    z_rand[2 + (n_s - 1) * 2 + 1] = 1

    z_rep = repair(z_rand, n_s, VARILLAS_POR_ANCHO[b][0], VARILLAS_POR_ANCHO[b][1])
    dec = decode(z_rep, n_s)
    print(f"diam_A={dec['diam_A']}, diam_B={dec['diam_B']}")
    print("corrido capa_0 oni:", dec['corrido'][0, :, 1])

    bars = get_active_bars(dec['corrido'], 'BOT', dec['diam_A'], dec['diam_B'], 60.0)
    print(f"Barras activas corrido BOT ({len(bars)}):")
    for b_ in bars:
        print(f"  y={b_[0]:.2f}cm, As={b_[1]}cm², ø{b_[2]}\"")

    # Round-trip
    enc = encode(dec, n_s)
    dec2 = decode(enc, n_s)
    assert np.array_equal(dec['corrido'], dec2['corrido']), "Round-trip FAIL"
